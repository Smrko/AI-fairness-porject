...
while count <= divide:
    ...
    # uniform sampling based on: https://github.com/sjessa/fair-ML/blob/6746dea2a07436eee0c5ea60be4311f7564df216/code/preprocessing.py
    df_predictor['label'] = target_train

    Sample_structure = pd.DataFrame({'group': [1, 1, 0, 0], 'label': [1, 0, 1, 0]})

    weights = [[(len(df_predictor[df_predictor[102] == s]) / (len(df_predictor)) * len(
        df_predictor[df_predictor['label'] == c]) / len(df_predictor))
                / (len(df_predictor[(df_predictor[102] == s) & (df_predictor['label'] == c)]) / len(df_predictor))
                for c in [1, 0]] for s in [1, 0]]

    sizes = [[len(df_predictor[(df_predictor[102] == s) & (df_predictor['label'] == c)]) for c in [1, 0]] for s in
             [1, 0]]

    Sample_structure['weight'] = [i for j in weights for i in j]
    Sample_structure['size'] = [i for j in sizes for i in j]
    # i put a regulator of 0.8 into the code to handel the strength of regulation
    Sample_structure = Sample_structure.assign(
        num=abs(round(0.8 * (Sample_structure["size"] - Sample_structure["weight"] * Sample_structure["size"]))))

    woman_desired = df_predictor[(df_predictor[102] == 0) & (df_predictor['label'] == 1)]
    woman_undesired = df_predictor[(df_predictor[102] == 0) & (df_predictor['label'] != 1)]
    man_desired = df_predictor[(df_predictor[102] != 0) & (df_predictor['label'] == 1)]
    man_undesired = df_predictor[(df_predictor[102] != 0) & (df_predictor['label'] != 1)]
    ...
    Uniform_Sampling_trainingset = df_prime.values
    predictor_Uniform_Sampling = Uniform_Sampling_trainingset[:, :103]
    target_Uniform_Sampling = Uniform_Sampling_trainingset[:, [103]]
    ...
    model = Sequential()
    n_cols = predictor_Uniform_Sampling.shape[1]
    input_shape = (n_cols,)
    print(n_cols)
    ...
    model.fit(predictor_Uniform_Sampling, target_Uniform_Sampling, epochs=500, batch_size=1000,
    ...
    value = [accrucy, accrucy_woman, accrucy_man, accrucy_white, accrucy_non_white, FPR_man, FPR_woman, FNR_man, FNR_woman, FPR_White, FPR_Non_white, FNR_white, FNR_Non_white, CV_Score_data, CV_Score_pred]
...
final_df.to_excel(writer, sheet_name='Uniform_Sampling')
writer.save()
