...
while count <= divide:
    ...
    scores_RF = model.predict_proba(validation)
    scores = scores_RF[:, [1]]
    correction_coeffizient = 0.55
    ...
    Reject_Option = np.append(XY, scores, axis=1)


    for i in range(np.shape(Reject_Option)[0]):
        if Reject_Option[i][2] == 0:
            if Reject_Option[i][5] >= 1 - correction_coeffizient:
                Reject_Option[i][5] = 1
            else:
                Reject_Option[i][5] = 0

        elif Reject_Option[i][2] == 1:
            if Reject_Option[i][5] <= correction_coeffizient:
                Reject_Option[i][5] = 0
            else:
                Reject_Option[i][5] = 1

    Analyse = Reject_Option[:, [0, 1, 2, 3, 5]]
...
final_df.to_excel(writer, sheet_name='ROC')
writer.save()
