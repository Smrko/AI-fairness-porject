...
while count <= divide:
    ...
    model.fit(predictor, target_train, epochs=500, batch_size=1000, validation_data=valid_set, callbacks=[stopping])
    ...
    FR_scores = model.predict(validation)
    FR_predictions = np.round(FR_scores)
    FR_predictions_shape = np.reshape(FR_predictions, (np.shape(predictions)[0], 1))
    ...
    XY = np.append(white, Z, axis=1)
    Discrimination_Aware_Ensemble_start = np.append(XY, FR_predictions_shape, axis=1)
    zero = np.zeros((np.shape(Discrimination_Aware_Ensemble_start)[0], 1))
    Discrimination_Aware_Ensemble = np.append(Discrimination_Aware_Ensemble_start, zero, axis=1)

    for i in range(np.shape(Discrimination_Aware_Ensemble)[0]):
        if Discrimination_Aware_Ensemble[i][2] == 0:
            if Discrimination_Aware_Ensemble[i][4] == 0 and Discrimination_Aware_Ensemble[i][5] == 0:
                Discrimination_Aware_Ensemble[i][6] = 0
            else:
                Discrimination_Aware_Ensemble[i][6] = 1
        elif Discrimination_Aware_Ensemble[i][2] == 1:
            if Discrimination_Aware_Ensemble[i][4] == 1 and Discrimination_Aware_Ensemble[i][5] == 1:
                Discrimination_Aware_Ensemble[i][6] = 1
            else:
                Discrimination_Aware_Ensemble[i][6] = 0

    Analyse = Discrimination_Aware_Ensemble[:, [0, 1, 2, 3, 6]]
...
final_df.to_excel(writer, sheet_name='Discrimination_Aware_Ensemble')
writer.save()
